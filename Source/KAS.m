...
KASLeftSide{StageId}(TargetConversionId, VelocityId) = log(InitialVelocities(VelocityId) ./ (TargetConversionsTemperatures{VelocityId, StageId}(TargetConversionId) .^ 2));
end
end
...
for TargetConversionId = 1:length(TargetConversions)
plot(SameConversionsReversedTemperatures{StageId}(TargetConversionId, :), KASLeftSide{StageId}(TargetConversionId, :));
[Slope, S] = polyfit(SameConversionsReversedTemperatures{StageId}(TargetConversionId, :), KASLeftSide{StageId}(TargetConversionId, :), 1);
...
ylabel('$\ln \frac {\beta}{T^{2}}$', 'Interpreter', 'LaTex', 'FontSize', 14);
...
PolyEaCoefficients{StageId} = polyfit(TargetConversions(:), Ea{StageId}(1, :), 8);
